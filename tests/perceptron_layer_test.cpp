...
void PerceptronLayerTest::test_calculate_combination_Hessian_form(void)
{
    ...
    if (numerical_differentiation_tests) {
        numerical_combination_Hessian_form = nd.calculate_Hessian_form(pl,
                                                                       &PerceptronLayer::calculate_combinations,
                                                                       inputs);

        assert_true((combination_Hessian_form[0] - numerical_combination_Hessian_form[0]).calculate_absolute_value() < 1.0e-3,
                    LOG);
        assert_true((combination_Hessian_form[1] - numerical_combination_Hessian_form[1]).calculate_absolute_value() < 1.0e-3,
                    LOG);
        assert_true((combination_Hessian_form[2] - numerical_combination_Hessian_form[2]).calculate_absolute_value() < 1.0e-3,
                    LOG);
        assert_true((combination_Hessian_form[3] - numerical_combination_Hessian_form[3]).calculate_absolute_value() < 1.0e-3,
                    LOG);
    }
...
void PerceptronLayerTest::test_calculate_combination_parameters_Jacobian(void)
{
    ...
    if (numerical_differentiation_tests) {
        numerical_combination_parameters_Jacobian = nd.calculate_Jacobian(pl,
                                                                          &PerceptronLayer::calculate_combinations,
                                                                          inputs,
                                                                          parameters);

        assert_true((combination_parameters_Jacobian - numerical_combination_parameters_Jacobian).calculate_absolute_value() < 1.0e-3,
                    LOG);
    }
...
void PerceptronLayerTest::test_calculate_combination_parameters_Hessian_form(void)
{
    ...
    if (numerical_differentiation_tests) {
        numerical_combination_parameters_Hessian_form = nd.calculate_Hessian_form(pl,
                                                                                  &PerceptronLayer::calculate_combinations,
                                                                                  inputs,
                                                                                  parameters);

        assert_true((combination_parameters_Hessian_form[0] - numerical_combination_parameters_Hessian_form[0]).calculate_absolute_value() < 1.0e-3,
                    LOG);
        assert_true((combination_parameters_Hessian_form[1] - numerical_combination_parameters_Hessian_form[1]).calculate_absolute_value() < 1.0e-3,
                    LOG);
        assert_true((combination_parameters_Hessian_form[2] - numerical_combination_parameters_Hessian_form[2]).calculate_absolute_value() < 1.0e-3,
                    LOG);
        assert_true((combination_parameters_Hessian_form[3] - numerical_combination_parameters_Hessian_form[3]).calculate_absolute_value() < 1.0e-3,
                    LOG);
    }
...
void PerceptronLayerTest::test_calculate_activation_derivative(void)
{
    ...
    if (numerical_differentiation_tests) {
        ...
        pl.set_activation_function(Perceptron::Threshold);
        activation_derivative = pl.calculate_activations_derivatives(combination);
        numerical_activation_derivative = nd.calculate_derivative(pl,
                                                                  &PerceptronLayer::calculate_activations,
                                                                  combination);
...
void PerceptronLayerTest::test_calculate_activation_second_derivative(void)
{
    ...
    if (numerical_differentiation_tests) {
        ...
        pl.set_activation_function(Perceptron::Threshold);
        activation_second_derivative = pl.calculate_activations_second_derivatives(combination);
        numerical_activation_second_derivative = nd.calculate_second_derivative(pl,
                                                                                &PerceptronLayer::calculate_activations,
                                                                                combination);
        assert_true((activation_second_derivative - numerical_activation_second_derivative).calculate_absolute_value() < 1.0e-3,
                    LOG);
...
void PerceptronLayerTest::test_calculate_Hessian_form(void)
{
    ...
    assert_true((pl.get_perceptron(0).calculate_Hessian(inputs) - Hessian_form[0]).calculate_absolute_value() < 1.0e-3,
                LOG);
    assert_true((pl.get_perceptron(1).calculate_Hessian(inputs) - Hessian_form[1]).calculate_absolute_value() < 1.0e-3,
                LOG);
...
void PerceptronLayerTest::test_calculate_parameters_Jacobian(void)
{
    ...
    if (numerical_differentiation_tests) {
        numerical_parameters_Jacobian = nd.calculate_Jacobian(pl,
                                                              &PerceptronLayer::calculate_outputs,
                                                              inputs,
                                                              parameters);
...
void PerceptronLayerTest::test_calculate_parameters_Hessian_form(void)
{
    ...
    if (numerical_differentiation_tests) {
        numerical_parameters_Hessian_form = nd.calculate_Hessian_form(pl,
                                                                      &PerceptronLayer::calculate_outputs,
                                                                      inputs,
                                                                      parameters);

        assert_true((parameters_Hessian_form[0] - numerical_parameters_Hessian_form[0]).calculate_absolute_value() < 1.0e-3,
                    LOG);
        assert_true((parameters_Hessian_form[1] - numerical_parameters_Hessian_form[1]).calculate_absolute_value() < 1.0e-3,
                    LOG);
    }
