...
/// @param document XML document containing the member data.
    ...
    void UnscalingLayer::from_XML(const tinyxml2::XMLDocument &document)
    {
        ...
        const tinyxml2::XMLElement *unscaling_neurons_number_element = root_element->FirstChildElement(
        ...
        for (size_t i = 0; i < unscaling_neurons_number; i++) {
            ...
            const tinyxml2::XMLElement *standard_deviation_element = statistics_element->FirstChildElement(
                    "StandardDeviation");
...
/// @param outputs_name Name of outputs from the unscaling layer. The size of this vector must be equal to the number of unscaling neurons.
    ...
    std::string UnscalingLayer::write_minimum_maximum_expression(const Vector<std::string> &inputs_name,
                                                                 const Vector<std::string> &outputs_name) const
    {
        const size_t unscaling_neurons_number = get_unscaling_neurons_number();
        ...
        for (size_t i = 0; i < unscaling_neurons_number; i++) {
            buffer.str("");
            buffer << "0.5*(" << inputs_name[i] << "+1.0)*(" << statistics[i].maximum << "-" << statistics[i].minimum << ")+" << statistics[i]
                    .minimum;
...
/// @param outputs_name Name of outputs from the unscaling layer. The size of this vector must be equal to the number of unscaling neurons.
    ...
    std::string UnscalingLayer::write_mean_stadard_deviation_expression(const Vector<std::string> &inputs_name,
                                                                        const Vector<std::string> &outputs_name) const
    {
        const size_t unscaling_neurons_number = get_unscaling_neurons_number();
...
/// @param outputs_name Name of outputs from the unscaling layer. The size of this vector must be equal to the number of unscaling neurons.
    ...
    std::string UnscalingLayer::write_expression(const Vector<std::string> &inputs_name,
                                                 const Vector<std::string> &outputs_name) const
    {
        switch (unscaling_method) {
        case NoUnscaling: {
