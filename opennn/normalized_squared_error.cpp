...
double NormalizedSquaredError::calculate_performance() const
{
...
double NormalizedSquaredError::calculate_selection_performance() const
{
...
#pragma omp parallel for private(i, selection_index, inputs, outputs, targets) reduction(+ : sum_squared_error, normalization_coefficient)
    ...
    if (normalization_coefficient < 1.0e-99) {
        ...
        buffer << "OpenNN Exception: NormalizedSquaredError class.\n"
               << "double calculate_selection_performance() const method.\n"
               << "Normalization coefficient is zero.\n"
               << "Unuse constant target variables or choose another error functional. ";
                                                                 ...
                                                                 const Vector<double> &target) const
{
    return (output - target) * 2.0 /
           target.calculate_sum_squared_error(data_set_pointer->calculate_training_target_data_mean());
}
...
std::string NormalizedSquaredError::write_performance_term_type() const
{
...
std::string NormalizedSquaredError::write_information() const
{
    std::ostringstream buffer;
