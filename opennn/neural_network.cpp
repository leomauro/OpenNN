...
/// The rest of members are initialized to their default values.
    ...
    NeuralNetwork::NeuralNetwork(void)
            : multilayer_perceptron_pointer(NULL), scaling_layer_pointer(NULL),
              principal_components_layer_pointer(NULL), unscaling_layer_pointer(NULL), bounding_layer_pointer(NULL),
              probabilistic_layer_pointer(NULL), conditions_layer_pointer(NULL), inputs_pointer(NULL),
              outputs_pointer(NULL), independent_parameters_pointer(NULL)
    {
        set_default();
    }
...
/// @param new_parameters New set of parameter values.
    ...
    if(size != parameters_number)
    {
        ...
        if (multilayer_perceptron_pointer && !independent_parameters_pointer) {// Only network parameters

            multilayer_perceptron_pointer->set_parameters(new_parameters);
        }
        else if (!multilayer_perceptron_pointer && independent_parameters_pointer) {// Only independent parameters

            independent_parameters_pointer->unscale_parameters(new_parameters);
        }
        else if (multilayer_perceptron_pointer && independent_parameters_pointer) {// Both network and independent parameters
...
/// Serializes the neural network object into a PMML document of the TinyXML library.
    ...
    tinyxml2::XMLDocument *NeuralNetwork::to_PMML(void) const
    {
        ...
        tinyxml2::XMLDeclaration *pmml_declaration = pmml_document->NewDeclaration(
...
/// Deserializes a TinyXML document into this neural network object.
        ...
        while (data_field) {
            ...
            if (is_probabilistic && data_field_optype == "categorical") {
                if (output_mining_fields.contains(field_name)) {
                    ...
                    while (data_field_value) {
                        const tinyxml2::XMLAttribute *attribute_value_data_field_value = data_field_value->FindAttribute(
                                "value");
            ...
            if (data_field_optype == "continuous") {
                ...
                if (data_field_interval) {
                    const tinyxml2::XMLAttribute *attribute_left_margin_interval = data_field_interval->FindAttribute(
                            "leftMargin");
                    const tinyxml2::XMLAttribute *attribute_right_margin_interval = data_field_interval->FindAttribute(
                            "rightMargin");
     ...
     */
        ...
        const tinyxml2::XMLElement *transformation_dictionary = root_element->FirstChildElement(
        ...
        if (is_probabilistic) {
            ...
            const tinyxml2::XMLAttribute *attribute_method_probabilistic_layer = neural_network->LastChildElement(
                    "NeuralLayer")->FindAttribute("normalizationMethod");
...
/// See the OpenNN manual for more information about the format of this element.
    ...
    tinyxml2::XMLDocument *NeuralNetwork::to_XML(void) const
    {
        ...
        if (multilayer_perceptron_pointer) {
            ...
            const tinyxml2::XMLElement *multilayer_perceptron_element = multilayer_perceptron_document->FirstChildElement(
        ...
        if (unscaling_layer_pointer) {
            ...
            const tinyxml2::XMLElement *unscaling_layer_element = unscaling_layer_document->FirstChildElement(
                    "UnscalingLayer");
        ...
        if (probabilistic_layer_pointer) {
            ...
            const tinyxml2::XMLElement *probabilistic_layer_element = probabilistic_layer_document->FirstChildElement(
                    "ProbabilisticLayer");
        ...
        if (bounding_layer_pointer) {
            ...
            const tinyxml2::XMLElement *bounding_layer_element = bounding_layer_document->FirstChildElement(
                    "BoundingLayer");
        ...
        if (conditions_layer_pointer) {
            ...
            const tinyxml2::XMLElement *conditions_layer_element = conditions_layer_document->FirstChildElement(
                    "ConditionsLayer");
        ...
        if (independent_parameters_pointer) {
            ...
            const tinyxml2::XMLElement *independent_parameters_element = independent_parameters_document->FirstChildElement(
