...
template<class T>
Matrix<T>::Matrix(const size_t &new_rows_number,
...
template<class T>
Matrix<T>::Matrix(const std::string &file_name)
        : std::vector<T>()
{
    rows_number = 0;
    columns_number = 0;
...
Matrix<T>::Matrix(const Matrix &other_matrix)
        : std::vector<T>(other_matrix.begin(), other_matrix.end())
{
                     ...
                     const Vector <size_t> &column_indices) const
{
    ...
    for (size_t j = 0; j < column_indices_size; j++) {
        ...
        for (size_t i = 0; i < row_indices_size; i++) {
            row_index = row_indices[i];
            standard_deviation[j] += ((*this)(row_index, column_index) - mean[j]) *
                                     ((*this)(row_index, column_index) - mean[j]);
        }
                                             ...
                                             const Vector <size_t> &row_indices)
{
    ...
    for (size_t j = 0; j < columns_number; j++) {
        ...
        for (size_t i = 0; i < rows_number; i++) {
            row_index = row_indices[i];
            (*this)(row_index, j) = 0.5 * ((*this)(row_index, j) + 1.0) * (statistics[j].maximum - statistics[j].minimum) + statistics[j].minimum;
        }
    }
}
                                                ...
                                                const Vector <size_t> &column_indices)
{
    ...
    for (size_t j = 0; j < column_indices.size(); j++) {
        ...
        if (statistics[column_index].maximum - statistics[column_index].minimum < 1e-99)
            continue;
        for (size_t i = 0; i < rows_number; i++) {
            (*this)(i, column_index) = 0.5 * ((*this)(i, column_index) + 1.0) * (statistics[column_index].maximum - statistics[column_index].minimum) + statistics[column_index].minimum;
        }
    }
}
...
{
    const Vector<T> column = arrange_column(column_index);
    const size_t new_rows_number = rows_number - column.count_less_than(minimum) - column.count_greater_than(maximum);
    Matrix<T> new_matrix(new_rows_number, columns_number);
    size_t row_index = 0;
    Vector<T> row(columns_number);
    for (size_t i = 0; i < rows_number; i++) {
