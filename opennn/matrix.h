...
/// This matrix also implements some mathematical methods which can be useful.
    ...
    public:
        ...
        Vector<double> calculate_mean_missing_values(const Vector <size_t> &,
                                                     const Vector <size_t> &,
        ...
        Vector <Statistics<T>> calculate_rows_statistics_missing_values(const Vector <size_t> &,
                                                                        const Vector <Vector<size_t>> &) const;
        ...
        Vector <Vector<double>> calculate_rows_shape_parameters_missing_values(const Vector <size_t> &,
                                                                               const Vector <Vector<size_t>> &) const;
        ...
        Vector <Histogram<T>> calculate_histograms_missing_values(const Vector <Vector<size_t>> &,
...
/// @param new_columns_number Number of columns in matrix.
    ...
    Matrix<T>::Matrix(const size_t &new_rows_number, const size_t &new_columns_number)
            : std::vector<T>(new_rows_number * new_columns_number)
    {
...
/// @param value Value of Type.
    ...
    Matrix<T>::Matrix(const size_t &new_rows_number, const size_t &new_columns_number, const T &value) : std::vector<T>(
            new_rows_number * new_columns_number)
    {
        if (new_rows_number == 0 && new_columns_number == 0) {
            rows_number = 0;
            columns_number = 0;
      ...
      if(column_indices[i] >= columns_number)
      {
        ...
        for (size_t j = 0; j < column_indices_size; j++) {
            ...
            for (size_t i = 0; i < row_indices_size; i++) {
                ...
                standard_deviation[j] += ((*this)(row_index, column_index) - mean[j]) * ((*this)(row_index,
                                                                                                 column_index) - mean[j]);
            }
        ...
        for (size_t j = 0; j < columns_number; j++) {
            if (statistics[j].standard_deviation < 1e-99) {
                ...
                for (size_t i = 0; i < row_indices.size(); i++) {
                    ...
                    (*this)(row_index, j) = ((*this)(row_index,
                                                     j) - statistics[j].mean) / statistics[j].standard_deviation;
                }
            }
        }
    }
      ...
      throw std::logic_error(buffer.str());
        ...
        for (size_t j = 0; j < columns_indices_size; j++) {
            if (statistics[j].standard_deviation < 1e-99) {
                ...
                for (size_t i = 0; i < rows_number; i++) {
                    (*this)(i, column_index) = ((*this)(i,
                                                        column_index) - statistics[j].mean) / statistics[j].standard_deviation;
                }
            }
        }
    }
      ...
      throw std::logic_error(buffer.str());
        ...
        for (size_t j = 0; j < columns_number; j++) {
            if (statistics[j].maximum - statistics[j].minimum < 1e-99) {
                ...
                for (size_t i = 0; i < rows_number; i++) {
                    (*this)(i, j) = 2.0 * ((*this)(i,
                                                   j) - statistics[j].minimum) / (statistics[j].maximum - statistics[j].minimum) - 1.0;
                }
            }
        }
    }
        ...
        for (size_t j = 0; j < columns_number; j++) {
            if (statistics[j].maximum - statistics[j].minimum < 1e-99) {
                ...
                for (size_t i = 0; i < row_indices_size; i++) {
                    ...
                    (*this)(row_index, j) = 2.0 * ((*this)(row_index,
                                                           j) - statistics[j].minimum) / (statistics[j].maximum - statistics[j]
                            .minimum) - 1.0;
                }
            }
        }
       ...
       throw std::logic_error(buffer.str());
        ...
        for (size_t j = 0; j < column_indices_size; j++) {
            ...
            if (statistics[j].maximum - statistics[j].minimum < 1e-99) {
                ...
                for (size_t i = 0; i < rows_number; i++) {
                    (*this)(i, column_index) = 2.0 * ((*this)(i, column_index) - statistics[j].minimum) / (statistics[j]
                                                                                                                   .maximum - statistics[j]
                                                                                                                   .minimum) - 1.0;
                }
            }
        }
        ...
        for (size_t j = 0; j < columns_number; j++) {
            if (statistics[j].standard_deviation < 1e-99) {
                ...
                for (size_t i = 0; i < rows_number; i++) {
                    ...
                    (*this)(row_index, j) = (*this)(row_index,
                                                    j) * statistics[j].standard_deviation + statistics[j].mean;
                }
            }
        }
    }
       ...
       throw std::logic_error(buffer.str());
        ...
        for (size_t j = 0; j < column_indices.size(); j++) {
            ...
            if (statistics[column_index].standard_deviation < 1e-99) {
                ...
                for (size_t i = 0; i < rows_number; i++) {
                    (*this)(i, column_index) = (*this)(i,
                                                       column_index) * statistics[column_index].standard_deviation + statistics[column_index]
                                                       .mean;
                }
            }
        }
      ...
      throw std::logic_error(buffer.str());
        ...
        for (size_t j = 0; j < columns_number; j++) {
            if (statistics[j].maximum - statistics[j].minimum < 1e-99) {
                ...
                for (size_t i = 0; i < rows_number; i++) {
                    (*this)(i, j) = 0.5 * ((*this)(i,
                                                   j) + 1.0) * (statistics[j].maximum - statistics[j].minimum) + statistics[j]
                                            .minimum;
                }
            }
        }
        ...
        for (size_t j = 0; j < columns_number; j++) {
            if (statistics[j].maximum - statistics[j].minimum < 1e-99) {
                ...
                for (size_t i = 0; i < rows_number; i++) {
                    ...
                    (*this)(row_index, j) = 0.5 * ((*this)(row_index, j) + 1.0) * (statistics[j].maximum - statistics[j]
                            .minimum)
       ...
       throw std::logic_error(buffer.str());
        ...
        for (size_t j = 0; j < column_indices.size(); j++) {
            ...
            if (statistics[column_index].maximum - statistics[column_index].minimum < 1e-99) {
                ...
                for (size_t i = 0; i < rows_number; i++) {
                    (*this)(i, column_index) = 0.5 * ((*this)(i,
                                                              column_index) + 1.0) * (statistics[column_index].maximum - statistics[column_index]
                            .minimum)
