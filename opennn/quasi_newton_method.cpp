...
/// Returns the method for approximating the inverse Hessian matrix to be used when training.
    ...
    const QuasiNewtonMethod::InverseHessianApproximationMethod &QuasiNewtonMethod::get_inverse_Hessian_approximation_method(
            void) const
    {
...
/// @param old_inverse_Hessian Inverse Hessian at the other point of the objective function.
            ...
            const Matrix<double> &old_inverse_Hessian) const
    {
        switch (inverse_Hessian_approximation_method) {
        case DFP: {
            return (calculate_DFP_inverse_Hessian(old_parameters,
                                                  parameters,
                                                  old_gradient,
                                                  gradient,
                                                  old_inverse_Hessian));
        ...
        case BFGS: {
            return (calculate_BFGS_inverse_Hessian(old_parameters,
                                                   parameters,
                                                   old_gradient,
                                                   gradient,
                                                   old_inverse_Hessian));
...
/// @param gradient The gradient of the objective function for the actual set of parameters.
    ...
    Matrix<double> QuasiNewtonMethod::calculate_DFP_inverse_Hessian(
            const Vector<double> &old_parameters,
            ...
            const Matrix<double> &old_inverse_Hessian) const
    {
        ...
        inverse_Hessian_approximation += parameters_difference.direct(parameters_difference) / parameters_difference.dot(
                gradient_difference);
