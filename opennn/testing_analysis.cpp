...
/// </ul>
TestingAnalysis::LinearRegressionResults TestingAnalysis::perform_linear_regression_analysis() const
{
    check();
...
Vector<Matrix<double>> TestingAnalysis::calculate_error_data() const
{
    ...
    for (size_t i = 0; i < outputs_number; i++) {
        ...
        // Percentage error
        error_data[i].set_column(2, (targets - outputs).calculate_absolute_value() * 100.0 /
                                    (outputs_maximum[i] - outputs_minimum[i]));
    }
...
Vector<double> TestingAnalysis::calculate_testing_errors() const
{
    ...
    if (testing_instances_number == 0) {
        ...
        buffer << "OpenNN Exception: TestingAnalysis class.\n"
               << "Vector< Matrix<double> > calculate_errors() const.\n"
               << "Number of testing instances is zero.\n";
...
/// </ul>
Vector<double> TestingAnalysis::calculate_pattern_recognition_testing_errors() const
{
...
double TestingAnalysis::calculate_testing_normalized_squared_error(const Matrix<double> &target_data,
                                                                   const Matrix<double> &output_data) const
{
    ...
    for (size_t i = 0; i < testing_instances_number; i++) {
        ...
        normalization_coefficient += target_data.arrange_row(i)
                                                .calculate_sum_squared_error(testing_target_data_mean);
    }
...
/// @target_data Testing target data.
/// @output_data Testing output data.
double TestingAnalysis::calculate_testing_cross_entropy_error(const Matrix<double> &target_data,
                                                              const Matrix<double> &output_data) const
{
    const size_t testing_instances_number = target_data.get_rows_number();
...
/// @target_data Testing target data.
/// @output_data Testing output data.
double TestingAnalysis::calculate_testing_weighted_squared_error(const Matrix<double> &target_data,
                                                                 const Matrix<double> &output_data) const
{
    const size_t testing_instances_number = target_data.get_rows_number();
    ...
    for (size_t i = 0; i < testing_instances_number; i++) {
        if (target_data(0, i) == 1.0) {
            error = positives_weight * output_data.arrange_column(i)
                                                  .calculate_sum_squared_error(target_data.arrange_column(i));
        } else if (target_data(0, i) == 0.0) {
            error = negatives_weight * output_data.arrange_column(i)
                                                  .calculate_sum_squared_error(target_data.arrange_column(i));
        } else {
            std::ostringstream buffer;
...
Matrix<size_t> TestingAnalysis::calculate_confusion_binary_classification(const Matrix<double> &target_data,
                                                                          const Matrix<double> &output_data,
                                                                          const double &decision_threshold) const
{
...
/// @param target_data Testing target data.
/// @param output_data Testing output data.
Matrix<size_t> TestingAnalysis::calculate_confusion_multiple_classification(const Matrix<double> &target_data,
                                                                            const Matrix<double> &output_data) const
{
    const size_t rows_number = target_data.get_rows_number();
...
/// <li> Total negatives
/// </ul>
Vector<size_t> TestingAnalysis::calculate_positives_negatives_rate(const Matrix<double> &target_data,
                                                                   const Matrix<double> &output_data) const
{
...
Matrix<size_t> TestingAnalysis::calculate_confusion() const
{
    ...
    if (inputs_number != variables.count_inputs_number()) {
        ...
        buffer << "OpenNN Exception: TestingAnalysis class." << std::endl
               << "Matrix<size_t> calculate_confusion() const method." << std::endl
    ...
    if (outputs_number == 1) {
        ...
        if (neural_network_pointer->get_probabilistic_layer_pointer() != NULL) {
            decision_threshold = neural_network_pointer->get_probabilistic_layer_pointer()
...
TestingAnalysis::RocAnalysisResults TestingAnalysis::perform_roc_analysis() const
{
    ...
    if (inputs_number != variables.count_inputs_number()) {
        ...
        buffer << "OpenNN Exception: TestingAnalysis class." << std::endl
               << "RocCurveResults perform_roc_analysis() const method." << std::endl
               << "Number of inputs in neural network must be equal to number of inputs in data set." << std::endl;
...
/// @param target_data Testing target data.
/// @param output_data Testing output data.
Matrix<double> TestingAnalysis::calculate_roc_curve(const Matrix<double> &target_data,
                                                    const Matrix<double> &output_data) const
{
...
/// @param target_data Testing target data.
/// @param output_data Testing output data.
double TestingAnalysis::calculate_area_under_curve(const Matrix<double> &target_data,
                                                   const Matrix<double> &output_data) const
{
    const Vector<size_t> positives_negatives_rate = calculate_positives_negatives_rate(target_data, output_data);
    const size_t total_positives = positives_negatives_rate[0];
    ...
    return sum / (double) (total_positives * total_negatives);
}

/// Returns the point of optimal classification accuracy, which is the nearest ROC curve point to the upper left corner (0,1).
/// @param target_data Testing target data.
/// @param output_data Testing output data.
double TestingAnalysis::calculate_optimal_threshold(const Matrix<double> &target_data,
                                                    const Matrix<double> &output_data) const
{
    const size_t rows_number = target_data.get_rows_number();
    const size_t columns_number = target_data.get_columns_number();
...
Matrix<double> TestingAnalysis::perform_cumulative_gain_analysis() const
{
    ...
    if (inputs_number != variables.count_inputs_number()) {
        ...
        buffer << "OpenNN Exception: TestingAnalysis class." << std::endl
               << "Matrix<double> perform_cumulative_gain_analysis() const method." << std::endl
               << "Number of inputs in neural network must be equal to number of inputs in data set." << std::endl;
...
/// @param target_data Testing target data.
/// @param output_data Testing output data.
Matrix<double> TestingAnalysis::calculate_cumulative_gain(const Matrix<double> &target_data,
                                                          const Matrix<double> &output_data) const
{
...
/// @param target_data Testing target data.
/// @param output_data Testing output data.
Matrix<double> TestingAnalysis::calculate_negative_cumulative_gain(const Matrix<double> &target_data,
                                                                   const Matrix<double> &output_data) const
{
...
Matrix<double> TestingAnalysis::perform_lift_chart_analysis() const
{
    ...
    if (inputs_number != variables.count_inputs_number()) {
        ...
        buffer << "OpenNN Exception: TestingAnalysis class." << std::endl
               << "Matrix<double> perform_lift_chart_analysis() const method." << std::endl
               << "Number of inputs in neural network must be equal to number of inputs in data set." << std::endl;
...
TestingAnalysis::KolmogorovSmirnovResults TestingAnalysis::perform_Kolmogorov_Smirnov_analysis() const
{
    ...
    if (inputs_number != variables.count_inputs_number()) {
        ...
        buffer << "OpenNN Exception: TestingAnalysis class." << std::endl
               << "Matrix<double> perform_Kolmogorov_Smirnov_analysis() const method." << std::endl
               << "Number of inputs in neural network must be equal to number of inputs in data set." << std::endl;
    ...
    Kolmogorov_Smirnov_results.positive_cumulative_gain =
            calculate_cumulative_gain(target_data, output_data);
    Kolmogorov_Smirnov_results.negative_cumulative_gain =
            calculate_negative_cumulative_gain(target_data, output_data);
...
Vector<double> TestingAnalysis::calculate_maximum_gain(const Matrix<double> &positive_cumulative_gain,
                                                       const Matrix<double> &negative_cumulative_gain) const
{
...
Matrix<double> TestingAnalysis::perform_calibration_plot_analysis() const
{
    ...
    if (inputs_number != variables.count_inputs_number()) {
        ...
        buffer << "OpenNN Exception: TestingAnalysis class." << std::endl
               << "Matrix<double> perform_calibration_plot_analysis() const method." << std::endl
               << "Number of inputs in neural network must be equal to number of inputs in data set." << std::endl;
...
/// @param target_data Testing target data.
/// @param output_data Testing output data.
Matrix<double> TestingAnalysis::calculate_calibration_plot(const Matrix<double> &target_data,
                                                           const Matrix<double> &output_data) const
{
...
Vector<Histogram<double>> TestingAnalysis::calculate_output_histogram(const Matrix<double> &output_data,
...
TestingAnalysis::BinaryClassifcationRates TestingAnalysis::calculate_binary_classification_rates() const
{
    ...
    if (inputs_number != variables.count_inputs_number()) {
        ...
        buffer << "OpenNN Exception: TestingAnalysis class." << std::endl
               << "BinaryClassificationRates calculate_binary_classification_rates() const method." << std::endl
               << "Number of inputs in neural network must be equal to number of inputs in data set." << std::endl;
    ...
    binary_classification_rates.true_positive_instances = calculate_true_positive_instances(target_data,
                                                                                            output_data,
                                                                                            testing_indices,
                                                                                            decision_threshold);
    binary_classification_rates.false_positive_instances = calculate_false_positive_instances(target_data,
                                                                                              output_data,
                                                                                              testing_indices,
                                                                                              decision_threshold);
    binary_classification_rates.false_negative_instances = calculate_false_negative_instances(target_data,
                                                                                              output_data,
                                                                                              testing_indices,
                                                                                              decision_threshold);
    binary_classification_rates.true_negative_instances = calculate_true_negative_instances(target_data,
                                                                                            output_data,
                                                                                            testing_indices,
                                                                                            decision_threshold);
...
/// @param decision_threshold Decision threshold.
Vector<size_t> TestingAnalysis::calculate_true_positive_instances(const Matrix<double> &target_data,
                                                                  const Matrix<double> &output_data,
...
/// @param decision_threshold Decision threshold.
Vector<size_t> TestingAnalysis::calculate_false_positive_instances(const Matrix<double> &target_data,
                                                                   const Matrix<double> &output_data,
                                                                   const Vector<size_t> &testing_indices,
                                                                   const double &decision_threshold) const
{
...
/// @param testing_indices Indices of the testing data
/// @param decision_threshold Decision threshold.
Vector<size_t> TestingAnalysis::calculate_false_negative_instances(const Matrix<double> &target_data,
                                                                   const Matrix<double> &output_data,
                                                                   const Vector<size_t> &testing_indices,
                                                                   const double &decision_threshold) const
{
...
/// @param testing_indices Indices of the testing data
/// @param decision_threshold Decision threshold.
Vector<size_t> TestingAnalysis::calculate_true_negative_instances(const Matrix<double> &target_data,
                                                                  const Matrix<double> &output_data,
                                                                  const Vector<size_t> &testing_indices,
                                                                  const double &decision_threshold) const
{
...
Matrix<Vector<size_t>> TestingAnalysis::calculate_multiple_classification_rates(const Matrix<double> &target_data,
                                                                                const Matrix<double> &output_data,
                                                                                const Vector<size_t> &testing_indices) const
{
    const size_t rows_number = target_data.get_rows_number();
...
Vector<Vector<double>> TestingAnalysis::calculate_error_autocorrelation(const size_t &maximum_lags_number) const
{
    ...
    if (inputs_number != variables.count_inputs_number()) {
        ...
        buffer << "OpenNN Exception: TestingAnalysis class." << std::endl
               << "Vector<double> calculate_error_autocorrelation() const method." << std::endl
               << "Number of inputs in neural network must be equal to number of inputs in data set." << std::endl;
...
Vector<Vector<double>> TestingAnalysis::calculate_input_error_cross_correlation(const size_t &maximum_lags_number) const
{
    ...
    if (inputs_number != variables.count_inputs_number()) {
        ...
        buffer << "OpenNN Exception: TestingAnalysis class." << std::endl
               << "Vector<double> calculate_input_error_cross_correlation() const method." << std::endl
               << "Number of inputs in neural network must be equal to number of inputs in data set." << std::endl;
    ...
    for (size_t i = 0; i < targets_number; i++) {
        ...
        input_error_cross_correlation[i] = input_column.calculate_cross_correlation(error.arrange_column(i),
                                                                                    maximum_lags_number);
    }
...
Vector<double> TestingAnalysis::calculate_binary_classification_tests() const
{
    ...
    if (!data_set_pointer) {
        ...
        buffer << "OpenNN Exception: TestingAnalysis class." << std::endl
               << "Vector<double> calculate_binary_classification_tests() const." << std::endl
    ...
    if ((true_positive + false_positive) * (true_positive + false_negative) *
        (true_negative + false_positive) * (true_negative + false_negative) == 0) {
...
double TestingAnalysis::calculate_logloss() const
{
    ...
    for (size_t i = 0; i < testing_instances_number; i++) {
        logloss += target_data(i, 0) * log(output_data(i, 0)) +
                   (1 - target_data(i, 0)) * log(1 - output_data(i, 0));
    }
...
std::string TestingAnalysis::to_string() const
{
    std::ostringstream buffer;
