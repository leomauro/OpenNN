...
Vector<Matrix<double>> TestingAnalysis::calculate_error_data(void) const
{
    ...
    for (size_t i = 0; i < outputs_number; i++) {
        ...
        error_data[i].set_column(1,
                                 (targets - outputs).calculate_absolute_value() / (outputs_maximum[i] - outputs_minimum[i]));

        // Percentage error

        error_data[i].set_column(2,
                                 (targets - outputs).calculate_absolute_value() * 100.0 / (outputs_maximum[i] - outputs_minimum[i]));
    }
...
//    const size_t columns_number = target_data.get_columns_number();
    ...
    const Matrix<double> sorted_target_data = sorted_target_output_data.arrange_submatrix_columns(
                columns_target_indices);
    const Matrix<double> sorted_output_data = sorted_target_output_data.arrange_submatrix_columns(
                columns_output_indices);
...
TestingAnalysis::KolmogorovSmirnovResults TestingAnalysis::perform_Kolmogorov_Smirnov_analysis(void) const
{
    ...
    Kolmogorov_Smirnov_results.negative_cumulative_gain = calculate_negative_cumulative_gain(target_data,
            output_data);
...
Vector<double> TestingAnalysis::calculate_binary_classification_tests(void) const
{
    ...
    if ((true_positive + false_positive) * (true_positive + false_negative) * (true_negative + false_positive) * (true_negative + false_negative) == 0) {
        ...
        Matthews_correlation_coefficient = (double)(true_positive * true_negative - false_positive * false_negative) / (double) sqrt(
                                               (double)((true_positive + false_positive) * (true_positive + false_negative) * (true_negative + false_positive) * (true_negative + false_negative)));
    }
...
double TestingAnalysis::calculate_logloss(void) const
{
    ...
    for (size_t i = 0; i < testing_instances_number; i++) {
        logloss += target_data(i, 0) * log(output_data(i, 0)) + (1 - target_data(i, 0)) * log(1 - output_data(i,
                   0));
    }
