        ...
        for (size_t j = 0; j < probabilistic_neurons_number; j++) {
            for (size_t k = 0; k < probabilistic_neurons_number; k++) {
                if (j == i && j == k && i == k) {
                    ...
                    Hessian_form[i](j,
                                    k) = outputs[i] * outputs[j] * outputs[k] - outputs[i] * outputs[j] * (1 - outputs[j]);
                } else if (j != i && i == k && j != k) {
                    Hessian_form[i](j,
                                    k) = outputs[i] * outputs[j] * outputs[k] - outputs[i] * outputs[j] * (1 - outputs[i]);
                } else if (j != i && i != k && j != k) {
...
void ProbabilisticLayer::from_XML(const tinyxml2::XMLDocument &document)
{
    ...
    // Probabilistic neurons number
    {
        const tinyxml2::XMLElement *element = probabilistic_layer_element->FirstChildElement(
