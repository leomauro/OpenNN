...
public:
                                                           ...
                                                           const double &x) const
    {
        ...
        const double d2 = (-y_forward_2 + 16.0 * y_forward - 30.0 * y + 16.0 * y_backward - y_backward_2) /
                          (12.0 * pow(h, 2));
        return d2;
    }
                                                         ...
                                                         const Vector<double> &x) const
    {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            y_forward_2i = (t.*f)(x_forward_2i);
            x_forward_2i[i] -= 2.0 * h_i;
            H(i, i) = (-y_forward_2i + 16.0 * y_forward_i - 30.0 * y + 16.0 * y_backward_i - y_backward_2i) /
                      (12.0 * pow(h_i, 2));
            for (size_t j = i; j < n; j++) {
                h_j = calculate_h(x[j]);
                x_backward_ij[i] -= h_i;
                ...
                x_forward_i_backward_j[i] -= h_i;
                x_forward_i_backward_j[j] += h_j;
                H(i, j) = (y_forward_ij - y_forward_i_backward_j - y_backward_i_forward_j + y_backward_ij) /
                          (4.0 * h_i * h_j);
            }
        }
        for (size_t i = 0; i < n; i++) {
    ...
    Matrix<double> calculate_forward_differences_Jacobian(const T &t,
                                                          Vector<double> (T::*f)(const size_t &,
                                                                                 const size_t &, const Vector<double> &) const,
    ...
    /// @param x: Input vector.
    template<class T>
    Matrix<Matrix<double>> calculate_central_differences_Hessian_matrices(const T &t,
                                                                          Vector<double> (T::*f)(const size_t &,
                                                                                                 const Vector<double> &,
                                                                                                 const Vector<double> &) const,
                                                                          const size_t &dummy_int,
