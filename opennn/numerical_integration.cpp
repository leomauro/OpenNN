...
std::string NumericalIntegration::write_numerical_integration_method() const
{
    ...
    default:
        {
            ...
            buffer << "OpenNN Exception: NumericalIntegration class.\n"
                   << "std::string write_numerical_integration_method() const method.\n"
...
void NumericalIntegration::set_numerical_integration_method(const NumericalIntegration::NumericalIntegrationMethod &new_numerical_integration_method)
{
...
double NumericalIntegration::calculate_Simpson_integral(const Vector<double> &x, const Vector<double> &y) const
{
    ...
    if (n % 2 != 0) {
        ...
        for (size_t i = 0; i < m; i++) {
            ...
            double wa = (c - a) / ((a - b) * (a - c)) * (1.0 / 3.0 * (a * a + c * c + a * c) -
                        0.5 * (a + c) * (b + c) + b * c);
            double wb = (c - a) / ((b - a) * (b - c)) * (1.0 / 3.0 * (a * a + c * c + a * c) -
                        0.5 * (a + c) * (a + c) + a * c);
            double wc = (c - a) / ((c - a) * (c - b)) * (1.0 / 3.0 * (a * a + c * c + a * c) -
                        0.5 * (a + c) * (a + b) + a * b);
