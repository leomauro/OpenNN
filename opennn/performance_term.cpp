 ...
 const Vector<Vector<double>> &layers_delta) const
{
    ...
    const Vector<Vector<double>> layers_inputs = multilayer_perceptron_pointer->arrange_layers_input(inputs,
                              layers_activation);

    const Vector<Matrix<double>> layers_combination_parameters_Jacobian = multilayer_perceptron_pointer->calculate_layers_combination_parameters_Jacobian(
                                  layers_inputs);
...
//    targets = data_set_pointer->get_instance(0, targets_indices);
    ...
    Vector<double> activations = neural_network_pointer->get_multilayer_perceptron_pointer()
                                 ->get_layer(0)
    ...
    const Vector<Vector<double>> &complete_layers_delta) const
{
    ...
    Matrix<double> output_layer_weights = multilayer_perceptron_pointer->get_layer(layers_number - 1)
                                          .arrange_synaptic_weights();
    ...
    const size_t layer_1_perceptrons_number = multilayer_perceptron_pointer->get_layer(index_1)
            .get_perceptrons_number();
    const size_t layer_2_perceptrons_number = multilayer_perceptron_pointer->get_layer(index_2)
            .get_perceptrons_number();
...
//                    std::cout << "Actual layers delta: " << multilayer_perceptron_pointer->get_layer(index_1).arrange_synaptic_weights().arrange_column(j).dot(layers_delta) << std::endl;
                    ...
                    interlayers_Delta(i, j) +=
                        ...
                        * multilayer_perceptron_pointer->get_layer(index_1)
                        .arrange_synaptic_weights()
                        .arrange_column(j)
                        .dot(layers_delta);
...
//                    std::cout << "--------" << std::endl;
                ...
                if (index_1 == 0 && index_2 == 0) {
                    ...
                    std::cout << "interlayers_combination_combination_Jacobian(0,0): " << interlayers_combination_combination_Jacobian(
                                  0,
                                  0) << std::endl;
                    ...
                    double interlayers_Delta02 =
                        ...
                        * layers_activation_derivative[0][j]
                        + layer_1_activation_second_derivative[i]
                        * interlayers_combination_combination_Jacobian(0, 0)
                        * multilayer_perceptron_pointer->get_layer(0)
                        .arrange_synaptic_weights()
                        .arrange_column(j)
                        .dot(layers_delta);
    ...
    for (int i = (int) layers_number - 1; i >= 0; i--) {
        for (int j = (int) layers_number - 1; j >= i; j--) {
            if (i == (int) layers_number - 1 && j == (int) layers_number - 1) { // Output-outputs layer
                interlayers_Delta(i,
                                  j) = calculate_output_interlayers_Delta(layers_activation_derivative[layers_number - 1],
                                          layers_activation_second_derivative[layers_number - 1],
                                          output_gradient,
                                          output_Hessian);
...
//   const size_t inputs_number = multilayer_perceptron_pointer->get_inputs_number();
    ...
    const size_t last_layer_parameters_number = multilayer_perceptron_pointer->get_layer(layers_number - 1)
            .count_parameters_number();
        ...
        for (size_t i = 0; i < parameters_number - last_layer_parameters_number; i++) {
            ...
            for (size_t j = parameters_number - last_layer_parameters_number; j < parameters_number; j++) {
                ...
                point_Hessian(i, j) =
                     ...
                     * layers_delta[layer_index_j][neuron_index_j]
                     * calculate_Kronecker_delta(parameter_index_j, neuron_index_i + 1)
                     * interlayers_combination_combination_Jacobian(layer_index_j - 1, layer_index_i)(
                         neuron_index_j,
                         neuron_index_i));
            }
        }
 ...
 const Matrix<double> &output_interlayers_Delta) const
{
    ...
    const size_t first_layer_parameters_number = multilayer_perceptron_pointer->get_layer(0)
            .arrange_parameters()
            .size();
    const size_t second_layer_parameters_number = multilayer_perceptron_pointer->get_layer(1)
            .arrange_parameters()
            .size();
