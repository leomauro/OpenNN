...
Vector<double> MeanSquaredError::calculate_output_gradient(const Vector<double> &output,
...
Matrix<double> MeanSquaredError::calculate_terms_Jacobian(void) const
{
    ...
    for (i = 0; i < (int) training_instances_number; i++) {
        ...
        if (!has_conditions_layer) {
            ...
            term = (particular_solution + homogeneous_solution * layers_activation[layers_number - 1] - targets) / sqrt(
