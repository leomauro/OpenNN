...
/// @param new_outputs_number Number of outputs neurons.
    ...
    MultilayerPerceptron::MultilayerPerceptron(const size_t &new_inputs_number,
...
/// @param perceptron_position Position on the perceptron in that layer.
    ...
    size_t MultilayerPerceptron::get_perceptron_index(const size_t &layer_index,
                                                      const size_t &perceptron_position) const
    {
        if (layer_index == 0) {
...
/// @param new_outputs_number Number of outputs from the multilayer perceptron.
    ...
    void MultilayerPerceptron::set(const size_t &new_inputs_number,
                                   const size_t &new_hidden_neurons_number,
                                   const size_t &new_outputs_number)
    {
        // Control sentence (if debug)
...
/// @param new_layer_perceptrons_number New numbers of neurons for that layer of the multilayer perceptron
    ...
    void MultilayerPerceptron::set_layer_perceptrons_number(const size_t &layer_index,
...
/// @param maximum Vector of maximum initialization values.
    ...
    void MultilayerPerceptron::randomize_parameters_uniform(const Vector<double> &minimum,
...
/// @param standard_deviation Vector of standard deviation values.
    ...
    void MultilayerPerceptron::randomize_parameters_normal(const Vector<double> &mean,
    ...
    Vector<Matrix<double> > MultilayerPerceptron::calculate_Hessian_form(const Vector<double> &inputs) const
    {
        ...
        for (size_t layer_index = layers_number - 2; layer_index != 0; layer_index--) {
            for (size_t output_index = 0; output_index < outputs_number; output_index++) {
                Hessian_form[output_index] = (layers_Jacobian[layer_index].calculate_transpose()).dot(Hessian_form[output_index])
                                                                                                 .dot(layers_Jacobian[layer_index]);

                for (size_t neuron_index = 0; neuron_index < layers_size[layer_index]; neuron_index++) {
                    Hessian_form[output_index] += layers_Hessian_form[layer_index][neuron_index] * Jacobian(output_index,
                                                                                                            neuron_index);
                }
            }
...
/// @param previous_layer_combination Combination vector of previous layer.
    ...
    Vector<double> MultilayerPerceptron::calculate_layer_combination_combination(const size_t &layer_index,
                                                                                 const Vector<double> &previous_layer_combination) const
    {
        ...
        const Vector<double> previous_layer_activation = layers[layer_index - 1].calculate_activations(
                previous_layer_combination);

        const Vector<double> layer_combination_combination = layers[layer_index].calculate_combinations(
...
/// @param previous_layer_activation_derivative Vector of activation derivatives of the previous layer.
    ...
    Matrix<double> MultilayerPerceptron::calculate_layer_combination_combination_Jacobian(const size_t &layer_index,
                                                                                          const Vector<double> &previous_layer_activation_derivative) const
    {
        ...
        Matrix<double> previous_layer_activation_Jacobian(previous_layer_perceptrons_number,
                                                          previous_layer_perceptrons_number,
                                                          0.0);
...
/// @param domain_layer_combination Combination vector of domain layer.
    ...
    Vector<double> MultilayerPerceptron::calculate_interlayer_combination_combination(const size_t &domain_layer_index,
                                                                                      ...
                                                                                      const Vector<double> &domain_layer_combination) const
    {
        ...
        if (domain_layer_index == image_layer_index) {
            ...
            for (size_t i = domain_layer_index + 1; i <= image_layer_index; i++) {
                interlayer_combination_combination = calculate_layer_combination_combination(i,
...
/// @param domain_layer_combination Vector of combination values of the domain layer.
    ...
    Matrix<double> MultilayerPerceptron::calculate_interlayer_combination_combination_Jacobian(const size_t &domain_layer_index,
                                                                                               const size_t &image_layer_index,
                                                                                               const Vector<double> &domain_layer_combination) const
    {
        ...
        if (domain_layer_index < image_layer_index) {
            ...
            layers_combination_combination_Jacobian[0] = calculate_layer_combination_combination_Jacobian(
                    domain_layer_index + 1,
                    layers[domain_layer_index].calculate_activations_derivatives(layers_combination[0]));
            layers_activation_combination[0] = layers[domain_layer_index].calculate_activations(layers_combination[0]);

            for (size_t i = 1; i < size; i++) {
                layers_combination[i] = layers[domain_layer_index + 1].calculate_combinations(
                        layers_activation_combination[i - 1]);

                layers_combination_combination_Jacobian[i] = calculate_layer_combination_combination_Jacobian(
                        domain_layer_index + 1 + i,
                        layers[domain_layer_index + i].calculate_activations_derivatives(layers_combination[i]));

                layers_activation_combination[i] = layers[domain_layer_index + i].calculate_activations(
            ...
            for (int i = (int) size - 2; i > -1; i--) {
                interlayer_combination_combination_Jacobian = interlayer_combination_combination_Jacobian.dot(
            ...
            interlayer_combination_combination_Jacobian.set(image_layer_perceptrons_number,
                                                            domain_layer_perceptrons_number,
                                                            0.0);
        }
...
/// @param layer_combinations Combination values of the layer with the previous index.
    ...
    Vector<double> MultilayerPerceptron::calculate_output_layer_combination(const size_t &layer_index,
...
/// @param layers_activation_derivative Activation derivative values for each layer.
    ...
    Vector<Matrix<double> > MultilayerPerceptron::calculate_output_layers_delta(const Vector<Vector<double> > &layers_activation_derivative) const
    {
        ...
        if (layers_number > 0) {
            ...
            output_layers_delta[layers_number - 1] = layers[layers_number - 1].arrange_activations_Jacobian(
                    layers_activation_derivative[layers_number - 1]);

            // Rest of layers

            for (int i = (int) layers_number - 2; i >= 0; i--) {
                output_layers_delta[i] = output_layers_delta[i + 1].dot(layers[i + 1].arrange_synaptic_weights())
...
/// @param output_layers_delta
             ...
             const Vector<Matrix<double> > &output_layers_delta) const
    {
        ...
        if (layers_number > 0) {
            ...
            output_interlayers_Delta(layers_number - 1,
                                     layers_number - 1) = layers[layers_number - 1].arrange_activations_Hessian_form(
                        ...
                        for (size_t i = 0; i < outputs_number; i++) {
                            for (size_t j = 0; j < layers_size[L]; j++) {
                                ...
                                for (size_t k = 0; k < layers_size[M]; k++) {
                                    ...
                                    for (size_t l = 0; l < layers_size[L + 1]; l++) {
                                        sum_2 += output_interlayers_Delta(L + 1, M)[i](l,
                                                                                       k) * layers_synaptic_weights[L + 1](
                                                l,
                                                j);
                                    }
...
/// @param parameters Vector of potential parameters for the multilayer perceptron.
    ...
    Vector<double> MultilayerPerceptron::calculate_outputs(const Vector<double> &inputs,
                                                           const Vector<double> &parameters) const
    {
        ...
        if (layers_number == 0) {
            ...
            for (size_t i = 1; i < layers_number; i++) {
                layer_parameters = parameters.take_out(layers_cumulative_parameters_number[i - 1],
...
/// The elements of that matrix are the partial derivatives of the outputs with respect to the multilayer perceptron parameters.
    ...
    Matrix<double> MultilayerPerceptron::calculate_Jacobian(const Vector<double> &inputs, const Vector<double> &) const
    {
        ...
        const Vector<Vector<Vector<double> > > first_order_forward_propagation = calculate_first_order_forward_propagation(
                inputs);
        ...
        const Vector<Matrix<double> > layers_combination_parameters_Jacobian = calculate_layers_combination_parameters_Jacobian(
                layers_inputs);
        ...
        for (size_t i = 1; i < layers_number; i++) {
            parameters_Jacobian = parameters_Jacobian.assemble_columns(output_layers_delta[i].dot(
...
/// @todo
    ...
    Vector<Matrix<double> > MultilayerPerceptron::calculate_Hessian_form(const Vector<double> &inputs,
                                                                         const Vector<double> &) const
    {
        ...
        const Vector<Vector<Vector<double> > > second_order_forward_propagation = calculate_second_order_forward_propagation(
                inputs);
        ...
        const Vector<Vector<Vector<double> > > perceptrons_combination_parameters_gradient = calculate_perceptrons_combination_parameters_gradient(
                layers_inputs);

        const Matrix<Matrix<double> > interlayers_combination_combination_Jacobian = calculate_interlayers_combination_combination_Jacobian(
                inputs);

        const Vector<Matrix<double> > output_layers_delta = calculate_output_layers_delta(layers_activation_derivative);

        const Matrix<Vector<Matrix<double> > > output_interlayers_Delta
                = calculate_output_interlayers_Delta(second_order_forward_propagation,
...
/// @param layers_activation Output values from each layer.
    ...
    Vector<Vector<double> > MultilayerPerceptron::arrange_layers_input(const Vector<double> &inputs,
...
/// @param first_order_forward_propagation Forward propagation quantities.
    ...
    Vector<Vector<double> > MultilayerPerceptron::arrange_layers_input(const Vector<double> &input,
                                                                       const Vector<Vector<Vector<double> > > &first_order_forward_propagation) const
    {
        const size_t layers_number = get_layers_number();
...
/// @param layers_inputs Input values for each layer.
    ...
    Vector<Matrix<double> > MultilayerPerceptron::calculate_layers_combination_parameters_Jacobian(const Vector<Vector<double> > &layers_inputs) const
    {
        ...
        for (size_t i = 0; i < layers_number; i++) {
            layers_combination_parameters_Jacobian[i] = layers[i].calculate_combinations_Jacobian(layers_inputs[i],
                                                                                                  dummy);
        }
    ...
    Vector<Vector<Vector<double> > > MultilayerPerceptron::calculate_perceptrons_combination_parameters_gradient(const Vector<Vector<double> > &layers_inputs) const
    {
        ...
        for (size_t i = 0; i < layers_number; i++) {
            ...
            for (size_t j = 0; j < layers_size[i]; j++) {
                ...
                perceptrons_combination_gradient[i][j] = perceptron.calculate_combination_gradient(layers_inputs[i],
...
/// @param inputs Set of inputs to the multilayer perceptron
    ...
    Vector<Vector<double> > MultilayerPerceptron::calculate_layers_activation_second_derivative(const Vector<double> &inputs) const
    {
        ...
        for (size_t i = 1; i < layers_number; i++) {
            ...
            layers_activation_second_derivatives[i] = layers[i].calculate_activations_second_derivatives(
                    layers_combination[i]);
        }
        ...
        for (size_t image_index = 0; image_index < layers_number; image_index++) {
            for (size_t domain_index = 0; domain_index < layers_number; domain_index++) {
                interlayers_combination_combination_Jacobian(image_index, domain_index) =
                        calculate_interlayer_combination_combination_Jacobian(domain_index,
                                                                              image_index,
...
/// @param inputs Vector of inputs to the multilayer perceptron
    ...
    Vector<Vector<Vector<double> > > MultilayerPerceptron::calculate_second_order_forward_propagation(const Vector<double> &inputs) const
    {
        ...
        for (size_t i = 1; i < layers_number; i++) {
            ...
            second_order_forward_propagation[2][i] = layers[i].calculate_activations_second_derivatives(
                    layers_combination[i]);
        }
...
/// @param document TinyXML document containing the member data.
    ...
    void MultilayerPerceptron::from_XML(const tinyxml2::XMLDocument &document)
    {
        ...
        // Layers activation function
        {
            const tinyxml2::XMLElement *layers_activation_function_element = root_element->FirstChildElement(
...
/// Deserializes a TinyXML document into this multilayer perceptron object.
    ...
    void MultilayerPerceptron::from_PMML(const tinyxml2::XMLElement *neural_network)
    {
        ...
        const tinyxml2::XMLAttribute *attribute_activation_function_neural_network = neural_network->FindAttribute(
                "activationFunction");
        ...
        for (size_t layer_i = 0; layer_i < number_of_layers; layer_i++) {
            const tinyxml2::XMLAttribute *attribute_activation_function_layer = neural_layer->FindAttribute(
                    "activationFunction");
            ...
            Matrix<double> current_layer_new_synaptic_weights(current_layer_perceptrons_number,
                                                              current_layer.get_perceptron(0).get_inputs_number());
...
/// @param outputs_name Name of output variables.
    ...
    std::string MultilayerPerceptron::write_expression(const Vector<std::string> &inputs_name,
