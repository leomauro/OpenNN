...
{
    return layers == other_multilayer_perceptron.layers &&
           display == other_multilayer_perceptron.display;
}
...
Vector<size_t> MultilayerPerceptron::arrange_layers_cumulative_parameters_number() const
{
    ...
    for (size_t i = 1; i < layers_number; i++) {
        layers_cumulative_parameters_number[i] = layers_cumulative_parameters_number[i - 1] +
...
size_t MultilayerPerceptron::get_layer_bias_index(const size_t &layer_index,
                                                  const size_t &perceptron_index) const
{
...
size_t MultilayerPerceptron::get_layer_synaptic_weight_index(const size_t &layer_index,
...
/// The size of this vector is the number of layers.
Vector<std::string> MultilayerPerceptron::write_layers_activation_function() const
{
    const size_t layers_number = get_layers_number();
...
void MultilayerPerceptron::set_layer_activation_function(const size_t &i,
...
Vector<Matrix<double>> MultilayerPerceptron::calculate_Hessian_form(const Vector<double> &inputs) const
{
    ...
    for (size_t layer_index = layers_number - 2; layer_index != 0; layer_index--) {
        for (size_t output_index = 0; output_index < outputs_number; output_index++) {
            Hessian_form[output_index] = layers_Jacobian[layer_index].calculate_transpose()
                                                                     .dot(Hessian_form[output_index])
                                                                     .dot(layers_Jacobian[layer_index]);
            for (size_t neuron_index = 0; neuron_index < layers_size[layer_index]; neuron_index++) {
                Hessian_form[output_index] += layers_Hessian_form[layer_index][neuron_index] *
                                                                             ...
                                                                             const Vector<double> &previous_layer_combination) const
{
    ...
    const Vector<double> previous_layer_activation = layers[layer_index - 1]
            .calculate_activations(previous_layer_combination);
    const Vector<double> layer_combination_combination = layers[layer_index]
            .calculate_combinations(previous_layer_activation);
                                                                                           ...
                                                                                           const Vector<double> &domain_layer_combination) const
{
    ...
    if (domain_layer_index < image_layer_index) {
        ...
        layers_combination_combination_Jacobian[0] =
                calculate_layer_combination_combination_Jacobian(domain_layer_index + 1,
        ...
        for (size_t i = 1; i < size; i++) {
            layers_combination[i] = layers[domain_layer_index + 1]
                    .calculate_combinations(layers_activation_combination[i - 1]);
            layers_combination_combination_Jacobian[i] =
                    calculate_layer_combination_combination_Jacobian(domain_layer_index + 1 + i,
                                                                     layers[domain_layer_index + i]
                                                                             .calculate_activations_derivatives(
                                                                                     layers_combination[i]));
            layers_activation_combination[i] = layers[domain_layer_index + i]
                    .calculate_activations(layers_combination[i]);
        }
        ...
        for (int i = (int) size - 2; i > -1; i--) {
            interlayer_combination_combination_Jacobian =
                    interlayer_combination_combination_Jacobian.dot(layers_combination_combination_Jacobian[i]);
        }
...
Vector<Matrix<double>> MultilayerPerceptron::calculate_output_layers_delta(const Vector<Vector<double>> &layers_activation_derivative) const
{
    ...
    if (layers_number > 0) {
        ...
        for (int i = (int) layers_number - 2; i >= 0; i--) {
            output_layers_delta[i] =
                    output_layers_delta[i + 1].dot(layers[i + 1].arrange_synaptic_weights())
                                              .dot(layers[i].arrange_activations_Jacobian(layers_activation_derivative[i]));
        }
    }
...
Matrix<Vector<Matrix<double>>> MultilayerPerceptron::calculate_output_interlayers_Delta(const Vector<Vector<Vector<double>>> &second_order_forward_propagation,
                                                                                        ...
                                                                                        const Vector<Matrix<double>> &output_layers_delta) const
{
    ...
    if (layers_number > 0) {
        ...
        output_interlayers_Delta(layers_number - 1, layers_number - 1)
                = layers[layers_number - 1]
                .arrange_activations_Hessian_form(layers_activation_second_derivative[layers_number - 1]);
        ...
        for (size_t i = 0; i < outputs_number; i++) {
            for (size_t j = 0; j < parameters_number; j++) {
                ...
                for (size_t k = j; k < parameters_number; k++) {
                    ...
                    if (layer_j != 0 && parameter_j != 0) {
                        // Neither the first layer nor parameter is a bias
                    }
                    if (layer_k != 0 && parameter_k != 0) {
                        parameters_Hessian_form[i](j, k) +=
                                output_layers_delta[layer_k](i, neuron_k)
                                * layers_activation_derivative[layer_k - 1][parameter_k - 1]
                                * interlayers_combination_combination_Jacobian(layer_k - 1, layer_j)(parameter_k - 1,
                                                                                                     neuron_j)
                                * perceptrons_combination_parameters_gradient[layer_j][neuron_j][parameter_j];
                    }
...
Matrix<Matrix<double>> MultilayerPerceptron::calculate_interlayers_combination_combination_Jacobian(const Vector<Vector<double>> &layers_combination) const
{
    ...
    for (size_t size_t i = 0; i < layers_number; i++) {
...
std::string MultilayerPerceptron::to_string() const
{
    std::ostringstream buffer;
...
Matrix<std::string> MultilayerPerceptron::write_information() const
{
    const size_t layers_number = get_layers_number();
    if (layers_number == 0) {
