...
GoldenSectionOrder::GoldenSectionOrderResults *GoldenSectionOrder::perform_order_selection()
{
    ...
    size_t ln = size_t(a + (1. - 0.618) * (b - a));
    size_t mu = size_t(a + 0.618 * (b - a));
    ...
    while (!end) {
        if (ln_performance[1] < mu_performance[1] || fabs(ln_performance[1] - mu_performance[1]) < tolerance) {
            b = mu;
            mu = ln;
            mu_performance = ln_performance;
            ln = size_t(a + (1. - 0.618) * (b - a));
            ln_performance = perform_model_evaluation(ln);
            current_training_performance = ln_performance[0];
            current_selection_performance = ln_performance[1];
            ln_parameters = get_parameters_order(ln);
    ...
    const size_t perceptrons_number = multilayer_perceptron_pointer->get_layer_pointer(last_hidden_layer)
